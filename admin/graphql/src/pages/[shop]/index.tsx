...
import { useDashboardOrderQuery, useOrderListViewMutation, useOrderstatusMutation } from '@/graphql/orders.graphql';
...
export default function ShopPage() {
  ...
  const ReviewCount = ReviewData?.reviews?.data.length;

  const { data: RefundData } = useRefundsQuery({
    skip: !Boolean(shopData?.shop?.id),
    variables: {
      shop_id: Number(shopData?.shop?.id),
      first: LIMIT,
      page: 1,
      orderBy: 'updated_at',
      sortedBy: 'DESC',
  ...
  console.log("redund:----", RefundData)
  const RefundCount = RefundData?.refunds?.data.length;

  console.log("data:------------", ReviewCount);

  const [orderstatus, { data }] = useOrderstatusMutation({});
  useEffect(() => {
    orderstatus({
      variables: {
  ...
  // useDashboardOrderQuery

  const [shopDateSelectionView, { data: DateSelection, loading: DateSelectionLoader }] = useOrderListViewMutation({})

  const orderCount = DateSelection?.shopDateSelectionView?.orderCount;
  const productCount = DateSelection?.shopDateSelectionView?.productCount;
  const reviewCount = DateSelection?.shopDateSelectionView?.reviewCount;
  const refundsCount = DateSelection?.shopDateSelectionView?.refundsCount;
  const grossSalesCount = DateSelection?.shopDateSelectionView?.grossSalesCount;
  const pendingOrdersCount = DateSelection?.shopDateSelectionView?.pendingOrdersCount;
  ...
  console.log("DateFilter", DateSelection);

  const PendingOrderData = data?.orderstatus?.length;
  console.log("PendingOrderDataLength", PendingOrderData);
  ...
  if (loading) return <Loader text={t('common:text-loading')} />;
  if (error) return <ErrorMessage message={error.message} />;
  if (DateSelectionLoader) return <Loader text={t('common:text-loading')} />;
    ...
    <div className="-m-5 md:-m-8">
      ...
      <div className="relative z-10 px-4 lg:px-6 xl:px-10">
        ...
        <div className="my-10 flex flex-wrap items-stretch gap-4 lg:gap-6 xl:gap-10">
          ...
          {/* Dashboard */}
          <div className="w-full flex-1 rounded-lg bg-white p-4 lg:p-6 xl:p-7 2xl:p-10">
            <div className='flex flex-row justify-end space-x-5 mb-5'>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 xl:gap-5 2xl:grid-cols-3 2xl:gap-7">
              <Link href={`/${shop}/products`}>
                <IconCard
                  content={isActualizar === true ? productCount : products_count?.toString() as any}
              ...
              <Link href={`/${shop}/orders`}>
                ...
                <IconCard
                  content={isActualizar === true ? orderCount : orders_count?.toString() as any}
                  title={t('common:text-total-orders')}
                  icon="OrdersIcon"
                  iconClassName="text-[#FF8D29]"
                  iconInnerClassName="bg-[#FFF0E2]"
              ...
              {role !== 'staff' ? (
                <>
                  ...
                  />
                  <IconCard
                    content={isActualizar === true ? grossSalesCount : totalEarnings?.toString() as any}
                    title={t('common:text-gross-sales')}
                    icon="GrossSaleIcon"
                    iconClassName="text-[#00AAFF]"
                    iconInnerClassName="bg-[#EFFAFF]"
                  ...
                  <Link href={`/${shop}/refunds`}>
                    <IconCard
                      content={isActualizar === true ? refundsCount : RefundCount?.toString() as any}
                      title={'Refund'}
                      icon="GrossSaleIcon"
                      iconClassName="text-[#0017E1]"
                      iconInnerClassName="bg-[#F0F2FF]"
                  ...
                  <Link href={`/${shop}/reviews`}>
                    ...
                    <IconCard
                      content={isActualizar === true ? reviewCount : ReviewCount?.toString() as any}

                      // content={ReviewCount === 0 ? '0' : ReviewCount as any}
                      title={'Review'}
                      icon="GrossSaleIcon"
                      iconClassName="text-[#0017E1]"
                      iconInnerClassName="bg-[#F0F2FF]"
                  ...
                  <Link href={`/${shop}/shop-pending-orders`}>
                    <IconCard
                      content={isActualizar === true ? pendingOrdersCount : PendingOrderData?.toString() as any}
                      title={'Pending Order'}
                      icon="OrdersIcon"
                      iconClassName="text-[#55AA22]"
                      iconInnerClassName="bg-[#F0F2FF]"
                      className='cursor-pointer'
                    />
                  </Link>
