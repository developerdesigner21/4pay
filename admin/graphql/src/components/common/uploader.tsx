...
import { Attachment, NewAttachment } from '__generated__/__types__';
...
export default function Uploader({
  ...
  const [files, setFiles] = useState<NewAttachment[]>(getPreviewImage(value));
  ...
  const thumbs = files?.map((file: any, idx) => {
    ...
    if (file.id) {
      ...
      if(splitArray[2] == "4pay.ai" && splitArray[3] != "backend"){
        splitArray[2] = "4pay.ai/backend";
        file.thumbnail = splitArray?.join('/');
      }
