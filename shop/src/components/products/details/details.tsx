...
const Details: React.FC<Props> = ({
  ...
  let previewImages = displayImage(selectedVariation?.image, gallery, image);
    ...
    try {
      if(Object.keys(selectedVariation).length){
        if (selectedVariation?.image) {
          previewImages = selectedVariation?.image; 
        }
      }  
    } catch (error) {}
  }
  ...
  return (
    <article className="rounded-lg bg-light">
      <div className="flex flex-col border-b border-border-200 border-opacity-70 md:flex-row mt-24">
      ...
      >
        ...
        <div className="text-sm text-body" dangerouslySetInnerHTML={{ __html: description }}></div>
      </Element>
{/* 
      <div className="border-b border-border-200 border-opacity-70 px-5 py-4 lg:px-16 lg:py-14">
        <h2 className="mb-4 text-lg font-semibold tracking-tight text-heading md:mb-6">Description</h2>
        <p className="text-sm text-body">Introducing your favorite Mamaearth Onion Hair Oil with a new & improved formula for even stronger, smoother, and shinier hair. With its lightweight and non-sticky formulation and oh-so-good fragrance, this natural and toxin-free hair loss oil helps you have stronger and healthier hair with the Goodness of Nature.Onion hair oil, rich in Sulphur, Potassium & antioxidants, reduces hair fall & accelerates hair regrowth. One of the newest breakthrough ingredients in hair growth, Redensyl, unblocks hair follicles & also boosts new hair growth. Safe for colored & chemically treated hair, the onion oil is free of harmful chemicals & toxins such as Silicones, Parabens, Mineral Oil, and Dyes.And because of our no toxins and no harmful chemicals philosophy, you won’t find any Silicones, Parabens, mineral oil & dyes in our organic onion hair oil.</p>
        <div className='flex flex-row justify-around py-5'>
          <div className='shadow-xl rounded-lg'>
            <Image
              src={previewImages[0].original}
              alt={name}
              width={250}
              height={450}
