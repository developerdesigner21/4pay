...
export default function GuestCheckoutPage() {
  ...
  const [billingAddress, setAddress] = useAtom(billingAddressAtom);
  const [shippingAddress] = useAtom(shippingAddressAtom);
  const router = useRouter();
  const { settings, isLoading } = useSettings();
  const guestCheckout = settings?.guestCheckout;

  const [Isbilling, useIsbilling] = useState({ istrue: true, isvalue: false });

  function newisbillingaction(){
    useIsbilling(item => ({...item, isvalue:item.isvalue?false:true}))
  }
  
  if(!Isbilling.isvalue){
        ...
        <div className="m-auto flex w-full max-w-5xl flex-col items-center rtl:space-x-reverse lg:flex-row lg:items-start lg:space-x-8">
          <div className="w-full space-y-6 lg:max-w-2xl">
            ...
            <GuestAddressGrid
              ...
              isbilling={Isbilling}
              isbillingaction={newisbillingaction}
            />
            <ScheduleGrid
              className="bg-light p-5 shadow-700 md:p-8"
