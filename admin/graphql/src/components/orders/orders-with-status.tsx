...
const bgColor: Record<string, string> = {
  'order-pending': 'bg-teal-100',
  'order-processing': 'bg-cyan-100',
  'order-completed': 'bg-emerald-100',
  'order-cancelled': 'bg-lime-100',
  'order-refunded': 'bg-green-100',
  'order-failed': 'bg-orange-100',
  'order-out-for-delivery': 'bg-gray-300',
  'order-at-local-facility': 'bg-amber-100',
};

const OrdersWithStatus = ({ orderStatus }: { orderStatus: string }) => {
  const [orderStatusFn, { data, loading }] = useOrderstatusMutation({
    ...
    <>
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-5">
          {data?.orderstatus?.map((order) => (
            <div
              key={order?.id + '' + order?.created_at}
              className={`${bgColor[order?.order_status!]} p-5 rounded-lg`}
            >
