...
import { useDateSelectionMutation, useOrderstatusMutation } from '@/graphql/orders.graphql';
import Loader from '@/components/ui/loader/loader';
import { useTranslation } from 'next-i18next';


export default function WebsiteCompleteOrderPage() {
    ...
    const [dateSelection, { data: SelectedData }] = useDateSelectionMutation({});

    const CompletedOrderData: any = data?.orderstatus;
    const SelectionDateFilter: any = SelectedData?.dateSelection;

    // console.log("SelectedData", CompletedOrderData, SelectionDateFilter?.length);
    ...
    return (
        <>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-14 py-5'>
                {isActualizar === true ?
                    (SelectionDateFilter?.map((item: any, index: any) => {
                        return (

                            <div className='relative' key={index}>
                                <div className='bg-[#009F7F1A] p-5 rounded-lg overflow-y-auto grow'>
                                    ...
                                    <div>
                                        ...
                                        {/* {isDisplaymore === false ?
                                    <div className='py-5 cursor-pointer flex flex-row justify-center' onClick={toggleReadMore} key={index}>
                                ...
                                {isDisplaymore === true ?
                                    <div className='py-3'>
                                        ...
                                        <div className='py-5 cursor-pointer flex flex-row justify-center' onClick={toggleViewLess} key={index}>
                                    ...
                                    {/* <div>
                                    <div className='py-5 cursor-pointer flex flex-row justify-center' onClick={() => toggleItemDetail(index)}>
                                        <p className='text-center text-sm'>{itemDetails[index] ? 'View Less' : 'View More'} </p> <Image src={Viewmoreicon} alt={''} className='-mt-0.5' />
                                    </div>
                                    {itemDetails[index] &&
                                        <div className='py-3'>
                                            <div className='space-y-3'>
                                    ...
                                    <button onClick={() => toggleItemDetail(index)}>
                                        {itemDetails[index] && 'View Less'}
                                    </button>
                                </div> */}
                                </div>
                            </div>
                    ...
                    (CompletedOrderData?.map((item: any, index: any) => {
                        return (
                            <div className='relative' key={index}>
                                <div className='bg-[#009F7F1A] p-5 rounded-lg overflow-y-auto grow'>
                                    <div className='flex felx-row justify-between justify-items-center'>
...
WebsiteCompleteOrderPage.authenticate = {
    permissions: adminOnly,
};
WebsiteCompleteOrderPage.Layout = Layout;
