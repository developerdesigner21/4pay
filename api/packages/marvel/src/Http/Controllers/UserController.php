...
class UserController extends CoreController
{
    ...
    public function customeotpverification(Request $request)
    {
        $profile = Profile::where('contact', $request->get('input')['number'])->first();
        if ($profile) {
            return [
                'isContactExist'=>true
            ];
    ...
    public function rcustomeotpverification(Request $request)
    {
        $profile = Profile::where('contact', $request->get('input')['number'])->first();
        if ($profile) {
            $user = User::where('id', $profile->customer_id)->first();
            ...
            $phoneNumber = $request->get('input')['number'];
    ...
    public function notificatontoken(Request $request)
    {
        $user = $request->user() ?? null;
        if ($user) {
            $userdata = User::where('id','=',$user['id'])->first();
            if ($userdata) {
                $userdata->notification_token = $request->input['token'];
                $userdata->save();
            }
        }
